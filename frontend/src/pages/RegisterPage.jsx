...
import { useUser } from "../context/UserContext"; // access backend functions
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await register({ name, email, password }); // call backend
      navigate("/user"); // redirect to user profile page
    } catch (err) {
      setError(err.message || "Registration failed"); // show error
    }
  };
  ...
  return (
    <section className="relative min-h-screen bg-black flex items-center justify-center px-6 md:px-12 overflow-hidden">
      ...
      >
        <h1 className="text-3xl font-bold text-white text-center mb-8 tracking-wide">Create Account</h1>
